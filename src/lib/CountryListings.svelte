...
<div
	...
	class:kramig-panda={itemType === 'baby'}
>
	<button
	...
	{#if isOpen}
		<div class="country-listings-content" out:slide={{ duration: 100 }}>
			{#if !available}
				<p>
					{toyName} seem to not be sold in {countryData.abbrv ??
						countryData.name}
				</p>
			{:else if countryData.cantCheckAutomatically}
				...
				{#if countryData.itemUrls?.[itemType]}
					<a href={countryData.itemUrls[itemType]}
						>See the listings for {toyName} in {countryData.abbrv ??
							countryData.name}</a
					>
				{/if}
			{:else}
				...
				{#if loaded}
					...
					{:then listings}
						...
						{:else}
							<p>No stores with {toyName} in stock 💔</p>
						{/if}
					{:catch err}
